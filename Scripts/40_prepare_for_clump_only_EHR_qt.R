...
## import header
hd               <- fread("zcat Weight_allchrs_min.tsv.gz | head")
#hd$EXTRA<- NULL
# Reorder columns
hd <- hd %>%
...
write.table(res.sentinel, "../24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_min.txt", sep="\t", row.names = F)

## mean ####
rm(list = ls())
#setwd("/sc-projects/sc-proj-computational-medicine/people/Aakash/12_GWAS_EHR_UKB/output/07_combined_allchrs/")
##################################################
####         import regional sentinels        ####
##################################################

## import header
hd               <- fread("zcat Weight_allchrs_mean.tsv.gz | head")
#hd$EXTRA<- NULL
# Reorder columns
hd <- hd %>%
  dplyr::select(CHROM, GENPOS, ID, ALLELE0, ALLELE1, A1FREQ, INFO, N, TEST, BETA, SE, CHISQ, LOG10P)
...
write.table(res.sentinel, "../24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_mean.txt", sep="\t", row.names = F)
...
## import header
hd               <- fread("zcat Weight_allchrs_median.tsv.gz | head")
#hd$EXTRA<- NULL
# Reorder columns
hd <- hd %>%
  dplyr::select(CHROM, GENPOS, ID, ALLELE0, ALLELE1, A1FREQ, INFO, N, TEST, BETA, SE, CHISQ, LOG10P)
...
write.table(res.sentinel, "../24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_median.txt", sep="\t", row.names = F)
...
write.table(res.sentinel, "../24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_max.txt", sep="\t", row.names = F)
