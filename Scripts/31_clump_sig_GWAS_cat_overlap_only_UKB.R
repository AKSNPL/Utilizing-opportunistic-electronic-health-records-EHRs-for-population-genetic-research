...
res.region_ukb <- fread("output/08_res_sentinels_all/Results.GWAS.UKB_only.202404m08.txt")# down
res.region_s_ukb<- fread("output/08_res_sentinels_all/Results.GWAS.all_chrs.20240317_ukb.txt")
#_ukb _small
...
res.region_s_ukb$id <- paste(res.region_s_ukb$id,"sukb",sep= "_")
...
res.region_merged<- rbind(res.region_s_ukb,res.region_ukb,res.region_min,res.region_max,res.region_mean,res.region_median)#29033
# Combine all data frames using union_all and remove duplicates
#res.region_merged <- union_all(
#  union_all(
#    union_all(
#      union_all(
#        union_all(res.region, res.region_ukb), res.region_min), res.region_max), res.region_mean), res.region_median
#) %>% distinct()
...
## combine
region     <- do.call(rbind, region)#446


# DONE
## write to file to extract proxies
#write.table(region, "output/08_res_sentinels_all/region.query_only_UKB.txt", sep="\t", col.names = F, row.names = F, quote = F)
...
jj <- dir(pattern="*LD.proxies*","./output/09_proxies/")#"*all_LD_only_UKB*"
...
#      1    1029    2158    2133    3211    4251   11013
#nacher:
# summary(res.region$R2.group)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#      1    1759    3648    3656    5475    7367     316 
...
## add rsIDs #155k snps
ukbb.snps  <- fread("//01_VEP_annotation/data/QCed.UKBB.variants.Berlin.txt")
dim(ukbb.snps)
...
source("/Aakash/12_GWAS_EHR_UKB/scripts/48_map_gwas_catalog.R")
...
## import VEP assignment from Hannah
...
## write to file
write.table(res.region, "/Aakash/12_GWAS_EHR_UKB/output/10_result_regions/Results.GWAS_only_UKB_genes20240408_new.txt", row.names = F, sep="\t")
