...
res.region_min <- fread("output/24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_min.txt")
res.region_max <- fread("output/24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_max.txt")
res.region_mean <- fread("output/24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_mean.txt")
res.region_median <- fread("output/24_res_sentinels_only_EHR_qt/Results.GWAS.all_chrs.20240503_median.txt")
...
res.region_merged<- rbind(res.region_min,res.region_max,res.region_mean,res.region_median)
...
write.table(region, "output/24_res_sentinels_only_EHR_qt/region.query_all_only_EHR_qt.txt", sep="\t", col.names = F, row.names = F, quote = F)
...
jj <- dir(pattern="*bla_ehr_only_qt_LD.*","./output/09_proxies/")
...
## write to file
write.table(res.region, "/Aakash/12_GWAS_EHR_UKB/output/10_result_regions/Results.GWAS_only_EHR_qt_genes20240503.txt", row.names = F, sep="\t")
