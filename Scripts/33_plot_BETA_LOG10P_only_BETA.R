...
tryCatch(
  expr = { 
    ...
    for (name in phenotypes){
      ...
      r2.sentinels<- fread("/Aakash/12_GWAS_EHR_UKB/output/10_result_regions/Results.GWAS_only_UKB_genes20240408_new.txt")
      ...
      r2.sentinels<- r2.sentinels %>% 
        dplyr::select(BETA,LOG10P,id,ID,R2.group)
      ...
      filtered_r2.sentinel1s <- r2.sentinels[!duplicated_groups, ] %>%
        filter(id==paste0(name,"_ukb"))
      
      ## Algorithm ####
      # load the data for the phenotypes
      data<- fread(paste0("output/20_combined_allchrs_UKB_only/",name,"_allchrs_.tsv.gz"))
      ...
      # select the required columns
      ll<- ll%>%
        dplyr::select(BETA,LOG10P,id,ID)
      ...
      # write the table
      write.table(main00, paste("output/11_BETA_LOG10P_plots/", paste0(name,"_pdata_only_UKB_md.txt"), sep="\t"))
      ...
      # Calculate the Pearson correlation coefficient and R^2
      correlation <- cor(merged_t$BETA.x, merged_t$BETA.y)
      ...
      p1 <- ggplot(merged_t, aes(x = LOG10P.x, y = LOG10P.y)) +
        ...
        labs(x = "LOG10P_EHR_md", y = "LOG10P_Only_UKB") +
      ...
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_LOG10P_only_UKB_md.png"), sep = ""), plot = p1, width = 10, height = 6, dpi = 500)
      
      p2 <- ggplot(merged_t, aes(x = BETA.x, y = BETA.y)) +
        geom_point(alpha = 0.5, color = "grey") +  # Plot all SNPs in grey
        geom_point(data = merged_t, aes(x = BETA.x, y = BETA.y), color = "black", size = 2) +  # Highlight hits in red
        geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add linear regression line
        labs(x = "BETA EHR Primary-Care md", y = "BETA UKB-Biomarker") +
        annotate("text", x = Inf, y = -Inf, label = paste("R:", round(correlation, 2), "\nR^2:", round(correlation_squared, 2)), 
                 hjust = 1.1, vjust = -0.1, size = 5, color = "black") +  # Add correlation text at bottom right
        theme_minimal() +  # Use a minimal theme with larger base size for better readability
        theme(
          panel.grid.major = element_line(color = "grey90"),  # Light grid lines
          panel.grid.minor = element_blank(),  # No minor grid lines
          strip.background = element_rect(fill = "lightblue", color = "grey80", size = 1),  # Facet strip background
          strip.text = element_text(size = 14, face = "bold"),  # Facet strip text
          plot.title = element_text(hjust = 0.5, size = 16, face = "bold")  # Centered plot title
        ) +
        ggtitle(unique(merged_t$id.x))
      #p2
      
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_BETA_only_UKB_md.png"), sep = ""), plot = p2, width = 10, height = 6, dpi = 500)
...
tryCatch(
  expr = { 
    ...
    for (name in phenotypes){
      ...
      # write the table
      write.table(main00, paste("output/11_BETA_LOG10P_plots/", paste0(name,"_pdata_mean_only_UKB.txt"), sep="\t"))
      ...
      p1 <- ggplot(merged_t, aes(x = LOG10P.x, y = LOG10P.y)) +
        geom_point() +
        geom_smooth(method = "lm", se = FALSE, color = "red") +
        labs(x = "LOG10P_EHR_mean", y = "LOG10P_Only_UKB")+
        facet_wrap(~id.x, scales = "free")
      ...
      #p2<- plot(merged_t$BETA.x, merged_t$BETA.y, 
      #     xlab = "BETA_EHR", ylab = "BETA_UKB")
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_LOG10P_mean_Only_UKB.png"), sep = ""), plot = p1, width = 10, height = 6, dpi = 500)
      
      p2 <- ggplot(merged_t, aes(x = BETA.x, y = BETA.y)) +
        geom_point(alpha = 0.5, color = "grey") +  # Plot all SNPs in grey
        geom_point(data = merged_t, aes(x = BETA.x, y = BETA.y), color = "black", size = 2) +  # Highlight hits in red
        geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add linear regression line
        labs(x = "BETA EHR Primary-Care mean", y = "BETA UKB-Biomarker") +
        annotate("text", x = Inf, y = -Inf, label = paste("R:", round(correlation, 2), "\nR^2:", round(correlation_squared, 2)), 
                 hjust = 1.1, vjust = -0.1, size = 5, color = "black") +  # Add correlation text at bottom right
        theme_minimal() +  # Use a minimal theme with larger base size for better readability
        theme(
        ...
        ggtitle(unique(merged_t$id.x))
      #p2
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_BETA_mean_Only_UKB.png"), sep = ""), plot = p2, width = 10, height = 6, dpi = 500)
...
tryCatch(
  expr = { 
    ...
    for (name in phenotypes){
      ...
      # write the table
      write.table(main00, paste("output/11_BETA_LOG10P_plots/", paste0(name,"_pdata_median_Only_UKB.txt"), sep="\t"))
      ...
      p1 <- ggplot(merged_t, aes(x = LOG10P.x, y = LOG10P.y)) +
        geom_point() +
        geom_smooth(method = "lm", se = FALSE, color = "red") +
        labs(x = "LOG10P_EHR_median", y = "LOG10P_Only_UKB")+
        facet_wrap(~id.x, scales = "free")
      ...
      #p2<- plot(merged_t$BETA.x, merged_t$BETA.y, 
      #     xlab = "BETA_EHR", ylab = "BETA_UKB")
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_LOG10P_median_Only_UKB.png"), sep = ""), plot = p1, width = 10, height = 6, dpi = 500)
      
      p2 <- ggplot(merged_t, aes(x = BETA.x, y = BETA.y)) +
        geom_point(alpha = 0.5, color = "grey") +  # Plot all SNPs in grey
        geom_point(data = merged_t, aes(x = BETA.x, y = BETA.y), color = "black", size = 2) +  # Highlight hits in red
        geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add linear regression line
        labs(x = "BETA EHR Primary-Care median", y = "BETA UKB-Biomarker") +
        annotate("text", x = Inf, y = -Inf, label = paste("R:", round(correlation, 2), "\nR^2:", round(correlation_squared, 2)), 
                 hjust = 1.1, vjust = -0.1, size = 5, color = "black") +  # Add correlation text at bottom right
        theme_minimal() +  # Use a minimal theme with larger base size for better readability
        theme(
        ...
        ggtitle(unique(merged_t$id.x))
      #p2
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_BETA_median_Only_UKB.png"), sep = ""), plot = p2, width = 10, height = 6, dpi = 500)
...
tryCatch(
  expr = { 
    ...
    for (name in phenotypes){
      ...
      # write the table
      write.table(main00, paste("output/11_BETA_LOG10P_plots/", paste0(name,"_pdata_min_Only_UKB.txt"), sep="\t"))
      ...
      p1 <- ggplot(merged_t, aes(x = LOG10P.x, y = LOG10P.y)) +
        geom_point() +
        geom_smooth(method = "lm", se = FALSE, color = "red") +
        labs(x = "LOG10P_EHR_min", y = "LOG10P_Only_UKB")+
        facet_wrap(~id.x, scales = "free")
      ...
      #p2<- plot(merged_t$BETA.x, merged_t$BETA.y, 
      #     xlab = "BETA_EHR", ylab = "BETA_UKB")
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_LOG10P_min_Only_UKB.png"), sep = ""), plot = p1, width = 10, height = 6, dpi = 500)
      
      p2 <- ggplot(merged_t, aes(x = BETA.x, y = BETA.y)) +
        geom_point(alpha = 0.5, color = "grey") +  # Plot all SNPs in grey
        geom_point(data = merged_t, aes(x = BETA.x, y = BETA.y), color = "black", size = 2) +  # Highlight hits in red
        geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add linear regression line
        labs(x = "BETA EHR Primary-Care min", y = "BETA UKB-Biomarker") +
        annotate("text", x = Inf, y = -Inf, label = paste("R:", round(correlation, 2), "\nR^2:", round(correlation_squared, 2)), 
                 hjust = 1.1, vjust = -0.1, size = 5, color = "black") +  # Add correlation text at bottom right
        theme_minimal() +  # Use a minimal theme with larger base size for better readability
        theme(
        ...
        ggtitle(unique(merged_t$id.x))
      #p2
      ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name,"_BETA_min_Only_UKB.png"), sep = ""), plot = p2, width = 10, height = 6, dpi = 500)
...
for (name in phenotypes) {
  ...
  tryCatch({
    ...
    # write the table
    write.table(main00, paste("output/11_BETA_LOG10P_plots/", paste0(name, "_pdata_max_Only_UKB.txt"), sep = "\t"))
    ...
    p1 <- ggplot(merged_t, aes(x = LOG10P.x, y = LOG10P.y)) +
      geom_point() +
      geom_smooth(method = "lm", se = FALSE, color = "red") +
      labs(x = "LOG10P_EHR_max", y = "LOG10P_Only_UKB") +
      facet_wrap(~id.x, scales = "free")
    
    ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name, "_LOG10P_max_Only_UKB.png"), sep = ""), plot = p1, width = 10, height = 6, dpi = 500)
    
    p2 <- ggplot(merged_t, aes(x = BETA.x, y = BETA.y)) +
      geom_point(alpha = 0.5, color = "grey") +  # Plot all SNPs in grey
      geom_point(data = merged_t, aes(x = BETA.x, y = BETA.y), color = "black", size = 2) +  # Highlight hits in red
      geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add linear regression line
      labs(x = "BETA EHR Primary-Care max", y = "BETA UKB-Biomarker") +
      annotate("text", x = Inf, y = -Inf, label = paste("R:", round(correlation, 2), "\nR^2:", round(correlation_squared, 2)), 
               hjust = 1.1, vjust = -0.1, size = 5, color = "black") +  # Add correlation text at bottom right
      theme_minimal() +  # Use a minimal theme with larger base size for better readability
      theme(
    ...
    ggsave(paste("output/11_BETA_LOG10P_plots/", paste0(name, "_BETA_max_Only_UKB.png"), sep = ""), plot = p2, width = 10, height = 6, dpi = 500)
  }, error = function(e) {
    message('Caught an error!')
    skip_to_next <<- TRUE
    # print(e)
  ...
  if (skip_to_next) {
    cat("Skipping to next phenotype due to error.\n")
    next
  }
}
