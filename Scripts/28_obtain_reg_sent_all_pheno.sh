...
#! set name
#SBATCH --output=/sc-projects/sc-proj-computational-medicine/people/Aakash/12_GWAS_EHR_UKB/logs/04_obtain_reg_sent_logs/COLLATE_ukb-%x-%j.out
...
cd /Aakash/12_GWAS_EHR_UKB/output/19_combined_allchrs_all_pheno
...
while read chr start end; do awk '{if(NR==1) print $0; else if(NR>1 && ($13+0.0) > 7.3) print $0}' OFS=" " ${out}_allchrs_.tsv | sed '1d' | awk -v chr=${chr} -v start=${start} -v end=${end} '{if(($1+0)==(chr+0) && ($2+0) >= (start+0) && ($2+0) <= (end+0)) print}' | awk -v start=${start} -v end=${end} -v max=0 '{if(sqrt(($10/$11)*($10/$11))>=max){want=$0; max=sqrt(($10/$11)*($10/$11))}} END{print want,start,end}' OFS=" " ; done < ${out}_regions.txt > ${out}_regional_sentinels_ukb.txt
